...
├── blocks/          # Reusable content blocks
    └── {blockname}/   - Individual block directory
        ...
        ├── {blockname}.model.json   # Per-block content model (source of truth → merged into component-models.json)
...
├── component-models.json  # Aggregated da.live content model (generated by /sync-models from blocks/*/*.model.json)
...
| --------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `/new-block <name>`   | Scaffolds all block files including `{name}.model.json`; run `/sync-models` after to update `component-models.json` |
| `/sync-models`        | Merges `blocks/*/{name}.model.json` files → `component-models.json`; cross-validates JS CONTENT_MODEL exports       |
